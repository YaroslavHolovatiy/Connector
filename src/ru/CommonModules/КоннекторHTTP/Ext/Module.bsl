...
//   ПараметрыЗапроса - СписокЗначений - параметры, которые будут отправлены в URL (часть после ?):
//     * Представление - Строка - ключ параметра в URL.
...
//    * ПараметрыЗапроса - СписокЗначений - см. описание Сессия.ПараметрыЗапроса.
...
Функция НовыеПараметры() Экспорт
	...
	Параметры.Вставить("ПараметрыЗапроса", Новый СписокЗначений);
...
Функция СоздатьСессию() Экспорт
	...
	Сессия.Вставить("ПараметрыЗапроса", Новый СписокЗначений);
...
Функция ПодготовитьЗапрос(Сессия, Метод, URL, ДополнительныеПараметры) Экспорт
	...
	ПараметрыЗапросаИзДополнительныхПараметров =
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыЗапроса", Новый СписокЗначений);
...
Функция ЗаполнитьПараметрыЗапроса(Путь)
	...
	Для Каждого СтрокаКлючРавноПараметр Из СтрРазделить(Запрос, "&", Ложь) Цикл
		...
		ПараметрНайден = Ложь;
		ИндексНайденногоПараметра = 0;
		Для Каждого ЭлементПараметров ИЗ ПараметрыЗапроса Цикл
			Если ЭлементПараметров.Представление = Ключ Тогда
				ПараметрНайден = Истина;
				ИндексНайденногоПараметра = ПараметрыЗапроса.Индекс(ЭлементПараметров);
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ПараметрНайден Тогда
			Если ТипЗнч(ПараметрыЗапроса[ИндексНайденногоПараметра].Значение) = Тип("Массив") Тогда
				ПараметрыЗапроса[ИндексНайденногоПараметра].Значение.Добавить(Значение);
			Иначе
				мЗначения = Новый Массив;
				мЗначения.Добавить(ПараметрыЗапроса[ИндексНайденногоПараметра].Значение);
				мЗначения.Добавить(Значение);
				ПараметрыЗапроса[ИндексНайденногоПараметра].Значение = мЗначения;
			КонецЕсли;
		Иначе
			ПараметрыЗапроса.Добавить(Значение, Ключ);
		КонецЕсли;
...
Функция КодироватьПараметрыЗапроса(ПараметрыЗапроса)
	...
	Для Каждого Параметр Из ПараметрыЗапроса Цикл
		...
		Если Параметр.Значение = Неопределено Тогда
			ЧастиПараметрыЗапроса.Добавить(Параметр.Представление);
		Иначе
			Для Каждого Значение Из Значения Цикл
				ЗначениеПараметра = КодироватьСтроку(Значение, СпособКодированияСтроки.КодировкаURL);
				ЧастиПараметрыЗапроса.Добавить(СтрШаблон("%1=%2", Параметр.Представление, ЗначениеПараметра));
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Функция КаноническиеПараметрыЗапросаAWS4(ПараметрыЗапроса)
	...
	Для Каждого ОчереднойПараметрЗапроса Из ПараметрыЗапроса Цикл
		Список.Добавить(ОчереднойПараметрЗапроса.Представление, СокрЛП(ОчереднойПараметрЗапроса.Значение));
	КонецЦикла;
	Список.СортироватьПоЗначению(НаправлениеСортировки.Возр);
	...
	Для Каждого ЭлементИсточника Из Источник Цикл
		...
		Если ТипЗнч(Приемник) = Тип("СписокЗначений") Тогда
			...
			Для Каждого ЭлементПриемника ИЗ Приемник Цикл
				...
				Если ЭлементПриемника.Представление = ЭлементИсточника.Представление Тогда
					ПараметрНайден = Истина;
					ИндексЭлементаПриемника = Приемник.Индекс(ЭлементПриемника);
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		Если ТипЗнч(Приемник) = Тип("Соответствие") ИЛИ ТипЗнч(Приемник) = Тип("Структура") Тогда
		...
		ИначеЕсли ТипЗнч(Приемник) = Тип("СписокЗначений") И ЭлементИсточника.Значение <> Неопределено Тогда
			
			Если ПараметрНайден Тогда
				Приемник[ИндексЭлементаПриемника].Значение = ЭлементИсточника.Значение;
			Иначе
